"""Extract credit-card charges into structured JSON rows."""
...
CHARGE_RE = re.compile(
    r"^(?P<date>\d{2}/\d{2})\s+(?P<merchant>.+?)\s+(?P<amount>\$[\d,]+\.\d{2})$"
)
REQUIRED_FIELDS = (
    "charge_date",
    "merchant",
...
def find_charge_lines(statement_text: str) -> list[str]:
    return [
        line.strip()
        for line in statement_text.splitlines()
        if CHARGE_RE.match(line.strip())
    ]


def load_prompt(prompt_path: Path, context: str, charge_line: str) -> str:
    return (
        prompt_path.read_text(encoding="utf-8")
        .replace("{{statement_context}}", context)
        .replace("{{charge_line}}", charge_line)
    )
...
def extract_charge(client: OpenAI, prompt: str) -> dict[str, Any]:
...
def main() -> None:
    ...
    pdf_paths = sorted(args.docs_dir.rglob("credit_card_jan2026.pdf"))
    if not pdf_paths:
        raise SystemExit(f"credit_card_jan2026.pdf not found in {args.docs_dir}")
    pdf_path = pdf_paths[0]
    statement_text = extract_pdf_text(pdf_path)
    charge_lines = find_charge_lines(statement_text)
    if not charge_lines:
        raise SystemExit(f"No charge lines found in {pdf_path}")

    prompt_path = Path(__file__).resolve().parent / "prompts" / "card_extract.md"
    context = statement_text.split("Date Merchant Category (issuer) Amount", 1)[0].strip()
    client = OpenAI(
    ...
    for charge_line in charge_lines:
        prompt = load_prompt(prompt_path, context, charge_line)
        rows.append(normalize_row(extract_charge(client, prompt)))
        print(f"Processed {charge_line}")
