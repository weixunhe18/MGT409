...
# Query parameter prevents Qt WebEngine from reusing a prior class exercise's
# cached Dash bundle when the local app keeps the same port.
CHAT_URL = "http://127.0.0.1:8050/?lecture=5"
...
class DesktopWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        browser = BrowserPane()
        chat = QWebEngineView()
        profile = chat.page().profile()
        profile.clearHttpCache()
        profile.cookieStore().deleteAllCookies()
        # Use a per-launch URL so Qt WebEngine cannot reuse a previous class
        # exercise's cached Dash document on the same localhost port.
        chat.setUrl(QUrl(f"{CHAT_URL}&session={time.time_ns()}"))
